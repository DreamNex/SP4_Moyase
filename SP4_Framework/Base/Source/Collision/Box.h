...
public:
	...
	{ 
		this->max = max;
		this->width = max.x - this->origin.x;
		this->height = max.y - this->origin.y;
	}
	void SetMin(Vector2 min)
	{
		this->min = min; 
		this->width = this->origin.x - this->min.x;
		this->height = this->origin.y - this->min.y;
	}
	void SetWidth(float width)
	{ 
		this->width = width; 
		this->max.x = this->origin.x + width;
		this->min.x = this->origin.x - width;
	}
	void SetHeight(float height)
	{
		this->height = height; 
		this->max.y = this->origin.y + height;
		this->min.y = this->origin.y - height;
	}
